...
export class UserDataService {
  ...
  /**
   * Get user's sensitive information with proper security controls
   * @param userId - The user ID
   * @param requesterRole - Role of the requesting user
   * @returns Promise resolving to redacted user data
   * @throws BadRequestException for invalid inputs
   * @throws UnauthorizedException for unauthorized access
   * @throws NotFoundException if user not found
   */
  async getUserSensitiveInfo(userId: string, requesterRole: string): Promise<Record<string, string>> {
    // Input validation
    ...
    if (requesterRole !== 'admin') {
      throw new UnauthorizedException('Insufficient permissions to access sensitive data');
    }
    
    // Find user with null/undefined check
    ...
    // Use prepared statement pattern for database queries
    const params = [userId];
    this.logger.debug('Fetching sensitive data with prepared statement');
    
    // Return only necessary data with last 4 digits for sensitive fields
    return {
      email: user.email,
  ...
  /**
   * Update user permissions with proper validation and error handling
  ...
  async updateUserPermissions(
    ...
    // Authorization check
    if (requesterRole !== 'admin') {
      throw new UnauthorizedException('Insufficient permissions to update user permissions');
    }
    ...
    try {
      ...
      // Log permission changes with appropriate level
      this.logger.log(`Updated permissions for user ${userId} by ${requesterRole}`);
